...
| 做不到 | 说明 |
|---|---|
| 可信时间戳（RFC 3161 TSA） | 签名里的时间取自**签名机的本地时钟，可被改**。要不可篡改的时间需外接 TSA 服务。折中：以业务系统服务端时间为准入库，PDF 里的时间只作展示 |
...
> ⚠️ **数字签名尚未接入测试台面板**——目前只能用代码调用验证。
> 要看效果：签一份 PDF 写到本地，用 **Adobe Acrobat**（不是浏览器内置阅读器）打开，
> 看顶部状态栏与签名面板。浏览器的 PDF 查看器多数不做签名验证，看不出区别。
