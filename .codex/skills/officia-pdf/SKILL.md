...
### 通用渲染的**已知失真**（如实告知，别对客户夸大）

7 份真实第三方 PDF 实测：版面、表格数字、行距、颜色都正确，但有三类失真：

| 失真 | 原因 | 表现 |
|---|---|---|
| **字形是近似的** | 用 officia 自己的字体栈画字，不光栅化 PDF 里嵌入的字体程序 | 笔画细节与原文件不同；**版面位置精确**（按 PDF 声明宽度横向缩放对齐） |
| **符号字体被画错** | 项目符号等符号字体的字符映射常不准 | 实测一份规范文档里的项目符号被画成 `♣` |
| **文本解码错误被忠实反映** | 部分 PDF 的 `ToUnicode` 表本身是错的 | 抽出来什么就画什么。先用 `extractText` 看一眼，乱码的话渲染也会乱 |

不支持：裁剪路径、内联图像、渐变、透明组与混合模式。绘制次序按类别
（底纹→图案→图像→线→文字）而非内容流原序——绝大多数文档文字在最上层，这个近似成立；
刻意把图盖在字上的会失真。

**性能参考**（实测，含解析）：46 页规范 4.0 s；164 页 14 s；341 页中文书 259 s。
扫描件快路则是另一个量级：29 页 0.2 s。
...
| 现象 | 原因 | 处置 |
|---|---|---|
| `toImages` 出的字笔画不像原文件 | 通用渲染用近似字形（不光栅化嵌入字体） | **属预期取舍**，见第六点五的失真表。版面位置是精确的 |
| `toImages` 把项目符号画成 `♣` 之类 | 符号字体的字符映射不准 | 已知失真，见第六点五 |
| `toImages` 出的正文是乱码 | 这份 PDF 的 `ToUnicode` 表本身错了 | 先 `extractText(pdf)` 验证——抽出来就乱说明是源文件问题，不是渲染引入的 |
