...
另外它还提供**反向转换 `toWord()` / `toWordEditable()`**：把电子版 PDF 转成 Word（见第七节），
...
> 190 个简单字体有 18 个如此）。officia 用**度量兼容**的替换族量宽并绘制
> （`Helvetica→Arial`、`Times-Roman→Times New Roman`、`Courier→Courier New`——
> 这几款当初就是照着彼此的度量设计的），版面对得上。
> 宽度仍属**近似**，且依赖本机可用字体。
> （2026-08-31 前有此缺陷：一行字全压在同一位置。）
...
**性能参考**（2026-08-31 实测，含解析，96 DPI，评估态限 30 页）：
...
**耗时主要由「实际渲染的页有多密」决定**，与文档总页数只弱相关（限页解析后）。
上表末行慢是因为那份规范每页都塞满表格——同为 30 张，简单页 160 ms/页、
密集页可达 2 s/页。测你自己的样本，别照搬。
...
// 要绝对坐标、不打算编辑时用（每块文字钉在原位置）
byte[] docx = OfficiaPdf.toWordPreserveLayout(pdf);
byte[] docx = OfficiaPdf.toWordEditable(pdf, "口令");
byte[] docx = OfficiaPdf.toWordEditable(new File("in.pdf"));
byte[] docx = OfficiaPdf.toWordEditable(inputStream);
OfficiaPdf.toWordEditable(new File("in.pdf"), new File("out.docx"));
```

> **默认就是兼顾版式与可编辑的那个**。1.1.4 起默认模式由 `TEXTBOX` 改为 `FLOW`：
> 几何参数（版心宽度、逐段行高、表格行高、单元格内边距、页边距）逐项按原文实测值还原后，
> 回环页数从 122 降到 **95**（原文 94 页、绝对定位模式 94 页）——版式差距 1 页，
> 可编辑性一分没丢。此前文档里「版式与可编辑无法兼得」的说法**已被实测推翻**。
>
> 只有「要逐页逐块对齐原 PDF、且完全不打算编辑」才用 `toWordPreserveLayout`。
...
| 模式 | 含义 | 状态 |
|---|---|---|
| `FLOW`（**默认**，1.1.4 起） | 段落 / 标题 / 表格还原成 Word 原生结构，**可自由编辑**；版式按原文实测值还原 | ✅ 可用（`toWord` / `toWordEditable`） |
| `TEXTBOX` | 每块文字钉在原坐标的文本框，位置精确到磅；代价是**基本不可编辑** | ✅ 可用（`toWordPreserveLayout`） |

**FLOW 模式产出什么**（实测 94 页招标文）：

| 项 | 结果 |
|---|---|
| 表格 | 真 `w:tbl` —— 可插入行列、拖列宽、格内打字；实测 13 张表（跨页续表已缝合，最长一张 63 行） |
| 跨页表格 | 自动缝成一张，重复表头只留一份并标 `w:tblHeader`（Word 在每个分页处自动重复） |
| 分隔线 | 没围成表格的独立横线还原成段落下边框（保留线宽与颜色）；PDF 用扁矩形画的线同样识别 |
| 加密 PDF | `/V 4 /R 4` + `/EncryptMetadata false`（阅读器能直接打开的那类）已支持 |
| 合并单元格 | `gridSpan` 37 处 + `vMerge` 258 处，逐行列数与 `tblGrid` 全部对齐 |
| 标题 | 带 `w:outlineLvl`，**Word 导航窗格可用**、能「引用 → 目录」一键生成目录 |
| 正文 | 普通段落，改字会重新断行（不是每行钉死） |
| 文字保真 | 与原 PDF 逐字符一致，差额只有被有意剥离的页码与重复表头副本 |
| 页眉页脚 | 按「位置在页边 + 内容模式跨页重复」双判据识别，页码改由页脚 `PAGE` 域生成 |
| 体积 | 90 KB（同文档 TEXTBOX 模式 234 KB） |
| 多栏 | 双栏 PDF 出**真正的 Word 分栏**（`w:cols`），阅读顺序按栏走；横跨两栏的图题 / 大表单独切成一栏的连续分节 |
| 西文词间空格 | 词间空隙常常不是空格字符而是 `TJ` 位移，已按几何间隔还原——不还原的话正文会是 `Figure1:Overallpre-training…` |

**FLOW 模式的代价**（如实告诉客户）：文字环绕会退化成顺序段落；跨页表格被切成多张独立表
（会尝试缝合，判据不满足时仍是两张）。要"看着最像原文"仍应选 `TEXTBOX`。

**版式还原到什么程度**（客户最常问的一条，给可核对的数字）：把产物再排成 PDF 与原文
逐行比对——八份真实样本的**横向偏移中位数全部为 0.0pt**（页边距与段落缩进都还原到位）；
纵向偏移中位数：企业档案管理规定 1.9pt、RFC 2616（114 页）**0.2pt**、询比文件 8.0pt、
招标文件 7.6pt，都在半行以内。页数：档案 / 红头公文 / 政府工作报告完全一致，
询比 56→57，招标 94→99，RFC 114→120。
断行位置也按原文还原：判「上一行有没有排到版心右边界」，没排满的是硬换行、各自成段，
排满的才并成段落交给 Word 重排（所以正文改字仍会重新流动）。

**页眉会重建**（1.1.4 起）：识别出的页眉写回 Word 的 header part（§17.10），页眉下面那条
分隔线还原成段落下边框——页眉<b>一个字都没有、只有一条线</b>的公文同样处理。实测一份 114 页的
英文规范，重建后每页文字的纵向偏移从 37.5pt 收到 5.3pt。

**页码会还原成域**：页眉页脚里的页码换成 Word 的 `PAGE` 域，其余文字照搬——
所以「第 X 页 共 Y 页」「- 3 -」「Fielding, et al ⇥ Standards Track ⇥ [Page 20]」
都能原样出来（含制表位的三段式也保得住），而且重新分页后页码自动是对的。

**页眉页脚的边界**：各只支持一种（取出现最多的那个模式），奇偶页不同眉、章节各自不同眉的
文档只还原其中一种。页码有偏移的文档（封面不计数，正文第 1 页印「第 1 页」但它是文档第 3 页）
不还原页码——`PAGE` 域给的是文档页号，强行还原会整体差一个偏移。

**插图会整块还原**：论文里的插图往往不是"一张图"，而是几十个矢量块 + 独立文字标签
拼出来的（实测某篇论文的一张图由 101 条线 + 63 个矩形 + 96 条路径 + 105 个文字片段构成）。
这类图会被识别成一个整体、连同标签渲染成一张图片插入，横跨两栏的还会自动单独切成一栏的节。
实测该论文 16 页转出后从 22 页降到 **18 页**，图本身与原文一致。

**插图识别的边界**：判据要求图区里有曲线或斜线（箭头、圆角），**纯直线画的流程图不算**——
这是为了保住表格：表格的线与矩形同样彼此紧挨，只按"挨在一起"聚类会把整张表当成图片，
真 `w:tbl` 就没了。

**多栏的边界**：栏数按整篇取主导值，不逐页分节（论文首页那种「单栏摘要 + 双栏正文」
混排页会跟着整篇走）。三栏及以上有通用实现但没有真实样本验证过。
...
| `TEXTBOX` 模式下产物是"真表格" | 该模式的表格线是逐条画出来的形状，**不能插入行列**。默认的 `FLOW` 模式产出的才是 `w:tbl` |
| 跨页单元格的文字断字 | 原文把一个格的文字分两页画时（如「联系方」+「式」），合并后仍是两行 |
| 页眉页脚独立成 part | 页码等仍作为正文段落出现，未放进 header/footer |
| 虚线样式 | 虚线会被画成实线 |
...
| 转出的 Word 里表格不能插入行列 | 用的是默认 `TEXTBOX` 模式，它把表格线画成形状 | 改用 `OfficiaPdf.toWordEditable(pdf)`（`FLOW` 模式），产出真 `w:tbl` |
| 转出的 Word 没有目录 / 导航窗格空白 | `TEXTBOX` 模式不推断标题层级 | 改用 `toWordEditable`：标题带大纲级别，导航窗格可用，也能「引用 → 目录」生成目录 |
| `FLOW` 模式版式不如 `TEXTBOX` 像原文 | 流式还原的固有代价（环绕 / 续表会走样） | 属既定取舍，不是缺陷；要版式保真就用 `toWord` |
| 双栏 PDF 转出来页数明显变多 | 页上有由几十个矢量块拼成的**复杂合成图**，判不出"通栏"，被挤进单栏 | 已知局限（纯文字的双栏页页数基本一致）；要版式保真就用 `toWord` |
| 产物顶部空一块 / 页眉没了 | 1.1.4 之前页眉被剥离但不写回 header part | 升级到 1.1.4 及以上 |
| 页眉页脚的页码不是原来的数字 | 属**预期**：还原成 `PAGE` 域，重新分页后自动是对的；照搬原数字反而会错 | — |
| 页码没还原、只剩固定文字 | 该文档页码有偏移（封面不计数），「数字==页号」不成立故不还原 | 已知边界 |
| 正文整体偏左或偏右一截 | 1.1.4 之前页边距与段落缩进用了两个不同的统计量，整篇会齐刷刷偏一个差值 | 升级到 1.1.4 及以上 |
| 英文 PDF 抽出来的字全粘在一起 | 1.1.4 之前 `TJ` 数组的位移调整量被丢弃，而西文词间空隙正是靠它表达 | 升级到 1.1.4 及以上 |
| 加粗的字重复了（「第第三三条条」） | 1.1.4 之前没认出「同一处画两遍」这种模拟加粗，两遍都抽了出来 | 升级到 1.1.4 及以上；修后这些字会被合并并正确标粗 |
| 插图区域出现大片纯黑 | **已知缺陷（未修）**：由线段围成的填充路径被一律退化成包围盒，横贯全图的一条深色折线因此被涂成一大块 | 同时影响 `toImages` 与转 Word 的插图；暂无绕法，如实告知客户 |
| 学术论文的表格转不出 `w:tbl` | 三线表只有横线没有竖线，表格识别靠横竖线构成的网格 | 已知边界；带完整边框的表格不受影响 |
