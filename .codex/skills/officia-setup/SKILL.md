...
```groovy
implementation 'plus.ruoyi:officia-all:1.0.0'
```
...
# 验证
ls ~/.m2/repository/plus/ruoyi/officia-all/1.0.0/
```
...
`mvn install` 需要下载插件时会卡住。绕过办法：用 JDK 自带 `jar` 手工把各模块 `target/classes` 打成 jar，按 Maven 仓库布局拷进 `~/.m2/repository/plus/ruoyi/<artifactId>/1.0.0/`，并补一个 `.pom` 文件（可从源码模块的 `pom.xml` 拷贝）。

布局要求：

```
~/.m2/repository/plus/ruoyi/officia-all/1.0.0/
├── officia-all-1.0.0.jar
└── officia-all-1.0.0.pom
```
...
| 症状 | 原因 | 处置 |
|---|---|---|
| `Could not resolve dependencies ... plus.ruoyi:officia-all:jar:1.0.0` | 本地仓没有 | 去 `../officia` 跑 `mvn install -DskipTests` |
