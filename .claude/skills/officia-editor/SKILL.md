...
- **`recalc` 用的就是 Officia.Cells 那 50 个函数的同一个引擎**，所以编辑器里看到的数字与
...
引擎**算不了**的公式例外——数组公式、引擎不支持的函数（如 `SUMPRODUCT`、`ROW`）、结构化引用、三维引用、外部工作簿引用，以及引用了本工作簿里没有的表的公式（跨表引用、整列整行引用本身照常重算）：带缓存值的保留 Excel 存下的结果、不重算，引用它的公式照常用这个值算，消息以「引擎不支持此公式写法，保留原缓存值」开头；没缓存值的才写 `#VALUE!`。
...
🔴 **公式结果不在浏览器里算**：同一个 50 函数引擎只有一份（在服务端），
