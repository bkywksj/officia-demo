...
- **`recalc` 用的就是 Officia.Cells 那 80 个函数的同一个引擎**，所以编辑器里看到的数字与
  `OfficiaCells.toPdf(xlsx)` 算出来的必然一致——浏览器端**没有**另写一套 JS 求值器。
- **`saveWorkbook` 不自动重算**。什么时候算是调用方的节奏问题；要落盘前兜底就自己串：
  `saveWorkbook(recalc(json))`。
- **打开再保存，公式不丢**：Excel 往下拖出来的**共享公式**，读入时展开成每格完整的公式；
  **数组公式**（Ctrl+Shift+Enter 输入的）带着数组标记（契约字段 `fa`）进出，`recalc` 按数组算、存回去仍是数组公式。
  早先的实现对这两类有静默丢失：共享公式的跟随格存回去只剩数字，数组公式存回去成了普通公式。

算不出来的格写成错误格，错误码与 Excel 一致（`#N/A`、`#DIV/0!`、`#REF!`、`#NUM!`，其余 `#VALUE!`），**不让整张表失败**；失败原因附在返回 JSON 的根级可选字段
`recalcErrors`（`[{sheet, ref, message}]`，无失败时不出现），界面据此提示「哪张表哪个格为什么算不出来」。
引擎**算不了**的公式例外——引擎不支持的函数（如 `OFFSET`、`INDIRECT`）、普通公式里的数组写法（Excel 那时走隐式交集）、结构化引用、三维引用、外部工作簿引用，以及引用了本工作簿里没有的表的公式（跨表引用、整列整行引用、数组公式本身照常重算）：带缓存值的保留 Excel 存下的结果、不重算，引用它的公式照常用这个值算，消息以「引擎不支持此公式写法，保留原缓存值」开头；没缓存值的才写 `#VALUE!`。
...
🔴 **公式结果不在浏览器里算**：同一个 80 函数引擎只有一份（在服务端），
...
- **复制 / 剪切 / 粘贴**（Ctrl+C / X / V）：表内粘贴带值、公式与格式，复制时公式的相对引用按位移改写，
  剪切是「搬」、公式不改写；从 Excel / 网页粘进来的制表符文本逐格按上面的输入规则识别。
  系统剪贴板里写的是显示文本，可直接粘到别处。暂不带合并区域；剪切后别处指向源区域的公式不跟着改指向
- **Delete** 清除整个选区的内容，格式保留
- **删除行 / 删除列**（「插入」页）：指向被删格的公式变 `#REF!`，跨过的区域收缩，合并区、行高、冻结随之调整
- **点列标 / 行号**选整列 / 整行（可拖动、可 Shift 扩选），左上角全选；整行整列上设格式记在行列样式上，不逐格铺满
- **键盘**：Shift+方向键扩选，Ctrl+方向键跳到数据边缘，Home / Ctrl+Home / Ctrl+End，PageUp / PageDown
...
| 态 | 含义 | 数量 |
|---|---|---|
| 正常（可点） | 模型里有这个东西，且**保存回目标格式时语义仍在** | Words 66 / Cells 47 |
| 琥珀（只读受限） | 只读透传 / 不属于文档内容 / 可编辑但导出时降级 | Words 1（水印）/ Cells 0 |
