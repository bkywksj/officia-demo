...
Could not resolve dependencies
  → plus.ruoyi:officia-license:jar:1.1.3 was not found in https://repo1.maven.org/maven2
```
...
curl -s -o /dev/null -w "%{http_code}\n" \
  https://repo1.maven.org/maven2/plus/ruoyi/officia-all/1.1.3/officia-all-1.1.3.pom      # 200
curl -s -o /dev/null -w "%{http_code}\n" \
  https://repo1.maven.org/maven2/plus/ruoyi/officia-license/1.1.3/officia-license-1.1.3.pom  # 404
```
...
**通常不用做这一步**——`officia-all:1.1.3` 自 2026-08-09 起已在 Maven Central，
...
# 验证
ls ~/.m2/repository/plus/ruoyi/officia-all/1.1.3/
```
...
`mvn install` 需要下载插件时会卡住。绕过办法：用 JDK 自带 `jar` 手工把各模块 `target/classes` 打成 jar，按 Maven 仓库布局拷进 `~/.m2/repository/plus/ruoyi/<artifactId>/1.1.3/`，并补一个 `.pom` 文件（可从源码模块的 `pom.xml` 拷贝）。

布局要求：

```
~/.m2/repository/plus/ruoyi/officia-all/1.1.3/
├── officia-all-1.1.3.jar
└── officia-all-1.1.3.pom
```
...
| 症状 | 原因 | 处置 |
|---|---|---|
| `Could not resolve dependencies ... plus.ruoyi:officia-all:jar:1.1.3` | 本地仓没有 | 去 `../officia` 跑 `mvn install -DskipTests` |
