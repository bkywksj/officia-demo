...
结果名从上传的源文件名派生——否则下载目录里全是 `words.pdf`、`words (1).pdf`、`words (4).pdf`，
根本对不上是哪个源文件转的。

| 情况 | 规则 | 例 |
|---|---|---|
| 换格式 | 只换后缀 | `年度报告.docx` → `年度报告.pdf`；`客户来信.eml` → `客户来信.pdf` |
| 同格式操作 | 加操作标记（否则与源同名，照样撞 `(1)`） | `合同.pdf` → `合同-水印.pdf` / `合同-加密.pdf` |
| 多份输入 | 首个源名 + 份数 | `合同.pdf` 等 3 份 → `合同等3份-合并.pdf` |
| 多份产出 | 加序号标记 | `年报.pdf` → `年报-第1页.pdf` … `年报-第5页.pdf` |
| 无源文件（CSV 文本框 / 内置样本） | 固定名 | `csv.pdf`、`样本-多页.pdf` |

> 源名里的路径前缀会被剥掉（个别客户端会把 `D:\download\合同.docx` 整段当文件名传上来）。
...
2. **参数选错不会报错**：语种（`lang`）与版面（`rotate` / `split`）不对时照样返回 200，
   ...
   同一本页面横放 + 两页并排扫描的书（183 页），同一份权重，**只差版面参数**：

   | 参数 | 产物 | 内容 |
   |---|---|---|
   | 不给 | 5.9 KB | 全是乱码（`效之芬实8名圣个要努实要会渝`） |
...
| Pdf | PDF → Word `toWord` | ZIP 容器 + 文本存活 |
| Pdf | 转 Word 产物容器完整性 | 5 个必需 OOXML 部件齐全 |
